...
public class LadyBird implements S_Observable{
	...
	private ArrayList<S_Observer> observers = new ArrayList<S_Observer>();
	...
	public void paint(Graphics g) {
		...
		g2.fillOval(x - settings.getHalfLadyBirdSize(), y
				- settings.getHalfLadyBirdSize(), 2 * settings
				.getHalfLadyBirdSize(), 2 * settings.getHalfLadyBirdSize());
		...
		for (int i = 1; i <= settings.getNumberOfDots(); i++) {
			g2.fillOval(x
					+ (int) (Math.round((settings.getHalfLadyBirdSize() / 1.5)
							* Math.cos(angle + i * a)))
					- settings.getHalfSizeOfSpot(), y
					+ (int) (Math.round((settings.getHalfLadyBirdSize() / 1.5)
							* Math.sin(angle + i * a)))
					- settings.getHalfSizeOfSpot(), 2 * settings
					.getHalfSizeOfSpot(), 2 * settings.getHalfSizeOfSpot());
		...
		g2.setStroke(new BasicStroke(2));
		g2.drawOval(x - settings.getHalfLadyBirdSize() + 1, y
				- settings.getHalfLadyBirdSize() + 1, 2 * settings
				.getHalfLadyBirdSize() - 1,
		...
		g2.drawLine(x
				...
				+ (int) (Math.round(settings.getHalfLadyBirdSize()
						* Math.sin(angle))), x
				+ (int) (Math.round(settings.getHalfLadyBirdSize() / 2.0
						* Math.cos(angle))), y
				+ (int) (Math.round(settings.getHalfLadyBirdSize() / 2.0
						* Math.sin(angle))));
	}
	...
	/**
	 * This method changes the ladybird's position depending on how the other
	 * ladybird interferes with this one.
	 * 
	 * @param other
	...
	public void collide(LadyBird other) {
		...
		if (Point2D.distance(other.getX(), other.getY(), x, y) < (other
			...
			double overLapAngle = Math.atan2(other.getX() - this.x, other
					.getY()
					- this.y);
			double overLap = (other.getSize() + settings.getHalfLadyBirdSize())
					- Point2D.distance(this.x, this.y, other.getX(), other
							.getY());
	...
	public void setSize(int size) {
		...
		settings = new LadyBirdSettings(size, settings.getColor(), settings
				.getDotColor());
	 ...
	 * @param dotColor
	 *            The color of the dots and border.
	 */
	public void setColors(Color color, Color dotColor) {

		settings = new LadyBirdSettings(settings.getHalfLadyBirdSize(), color,
				dotColor);
		fireChange();
	}
	...
	public void setSettings(LadyBirdSettings newSettings) {
