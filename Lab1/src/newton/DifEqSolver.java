...
class DifEqSolver {
	...
	private static double rky3(double[] x, double t, double m0, double m1) {
	  return -K * (m0 + m1) * x[0] / Math.pow(x[0] * x[0] + x[1] * x[1] + x[2] * x[2], 1.5);
	}

	private static double rky4(double[] x, double t, double m0, double m1) {
	  return -K * (m0 + m1) * x[1] / Math.pow(x[0] * x[0] + x[1] * x[1] + x[2] * x[2], 1.5);
	}

	private static double rky5(double[] x, double t, double m0, double m1) {
	  return -K * (m0 + m1) * x[2] / Math.pow(x[0] * x[0] + x[1] * x[1] + x[2] * x[2], 1.5);
	}

	private static void RungeKutta(double[] x0, double[] xf, double t, double h, double m0, double m1) {
	...
	public static void solve(double[] x00, double []v00,
	                         double[] x10, double[] v10,
	                         double m0, double m1, double t, double h,
	                         double[] x0f, double[] v0f,
	                         double[] x1f, double[] v1f) {
	  x0[0] = x10[0] - x00[0];
	  x0[1] = x10[1] - x00[1];
